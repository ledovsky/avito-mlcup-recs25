...
def get_node_features(df_clickstream: pl.DataFrame, df_cat: pl.DataFrame) -> pl.DataFrame:
    # compute per-node conversion rate (contacts / non-contacts)
    item_stats = (
        df_clickstream
        .group_by("node")
        .agg(
            total_views=pl.count(),
            contact_count=pl.col("is_contact").sum(),
        )
        .with_columns([
            (pl.col("total_views") - pl.col("contact_count")).alias("non_contact_count"),
            pl.when(pl.col("total_views") - pl.col("contact_count") > 0)
              .then(pl.col("contact_count") / (pl.col("total_views") - pl.col("contact_count")))
    ...
    category_contacts = (
        df_with_cat
        .group_by("category")
        .agg(category_contact_count=pl.col("is_contact").sum())
    )
    total_contacts = category_contacts["category_contact_count"].sum()
    category_share = (
        category_contacts
        .with_columns(
            (pl.col("category_contact_count") / total_contacts)
            .alias("category_contact_share")
        )
    ...
    features = (
        ...
        .select(["node", "category", "node_view_contact_rate", "category_contact_share"])
    )
    return features


def get_user_category_features(df_clickstream: pl.DataFrame, df_cat: pl.DataFrame) -> pl.DataFrame:
    # join clickstream with category info
    df_with_cat = df_clickstream.join(df_cat.select(["item", "category"]), on="item", how="inner")
    # compute per-user-category share of contacts
    user_cat_stats = (
        df_with_cat
        .group_by("cookie", "category")
        .agg(
            total_events=pl.count(),
            contact_count=pl.col("is_contact").sum()
        )
        .with_columns(
            (pl.col("contact_count") / pl.col("total_events"))
            .alias("user_category_contact_share")
        )
    )
    # rename and select final columns
    return (
        user_cat_stats
        .rename({"cookie": "user_id", "category": "category_id"})
...
def enrich(
    ...
    # compute target per user-node pair: 10 if any contact, 1 if any non-contact, else 0
    df_click_agg = (
        df_clickstream
        .group_by(["cookie", "node"])
        .agg(
            contact_sum=pl.col("is_contact").sum(),
            total_events=pl.count()
        )
        .with_columns([
            (pl.col("total_events") - pl.col("contact_sum")).alias("non_contact_sum"),
    ...
    # enrich with target
    df = df.join(
        df_click_agg.rename({"cookie": "user_id"}),
        on=["user_id", "node"],
        how="left"
    )
