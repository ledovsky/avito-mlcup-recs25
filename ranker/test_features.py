...
from ranker.features import get_node_features, get_user_category_features, enrich, add_target
...
def test_get_node_features():
    ...
    assert features["category_contact_share"].to_list() == pytest.approx(
        expected_shares
    )
...
def test_get_user_category_features():
    ...
    features = get_user_category_features(df_click, df_cat).sort(
        ["user_id", "category_id"]
    )
    ...
    assert features["user_category_contact_share"].to_list() == pytest.approx(
        expected_share
    )
...
def test_enrich():
    ...
    df_cat = pl.DataFrame({"item": [1, 2], "node": [10, 20], "category": [100, 200]})
...
def test_add_target():
    ...
    result = add_target(df, df_click)
    assert result["target"].to_list() == [10, 1]
